...
struct PrivacyPolicyView: View {
    ...
    private var privacyBody: String {
        ...
        case "it":
            return """
PINO salva i punti che crei su questo iPhone e Apple Watch. Posizioni, nomi, categorie e indirizzi restano sui tuoi dispositivi e si sincronizzano tra loro con WatchConnectivity. Non vengono inviati a un server di PINO.
...
PINO non usa pubblicità, SDK di analisi né tracciamento. Non c’è un account. Eliminando l’app, o i singoli punti in elenco, i dati spariscono dal dispositivo.
...
"""
        case "de":
            return """
PINO speichert Pins, die du auf diesem iPhone und Apple Watch anlegst. Orte, Namen, Kategorien und Adressen bleiben auf deinen Geräten und synchronisieren sich per WatchConnectivity. Sie werden nicht auf einen PINO-Server hochgeladen.
...
"""
        case "es":
            return """
PINO guarda los puntos que creas en este iPhone y Apple Watch. Ubicaciones, nombres, categorías y direcciones permanecen en tus dispositivos y se sincronizan con WatchConnectivity. No se envían a un servidor de PINO.
...
"""
        case "fr":
            return """
PINO enregistre les points que tu crées sur cet iPhone et Apple Watch. Lieux, noms, catégories et adresses restent sur tes appareils et se synchronisent via WatchConnectivity. Ils ne sont pas envoyés vers un serveur PINO.
...
"""
        default:
            return """
PINO saves pins you create on this iPhone and Apple Watch. Locations, names, categories, and addresses stay on your devices and sync between them with WatchConnectivity. They are not uploaded to a PINO server.
...
PINO does not use advertising, analytics SDKs, or tracking. There is no account. Deleting the app, or deleting pins in the list, removes that data from the device.
