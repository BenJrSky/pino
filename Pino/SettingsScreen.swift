...
struct PrivacyPolicyView: View {
    ...
    private var privacyBody: String {
        ...
        case "it":
            return """
PinO salva i punti che crei su questo iPhone e Apple Watch. Posizioni, nomi, categorie e indirizzi restano sui tuoi dispositivi e si sincronizzano con l’Apple Watch con WatchConnectivity. Non vengono inviati a un server di PinO.
...
PinO non usa pubblicità, SDK di analisi né tracciamento. Non c’è un account. Eliminando l’app i dati spariscono dal dispositivo.
...
"""
        case "de":
            return """
PinO speichert Pins, die du auf diesem iPhone und Apple Watch anlegst. Orte, Namen, Kategorien und Adressen bleiben auf deinen Geräten und werden per WatchConnectivity mit der Apple Watch synchronisiert. Sie werden nicht auf einen PinO-Server hochgeladen.
...
"""
        case "es":
            return """
PinO guarda los puntos que creas en este iPhone y Apple Watch. Ubicaciones, nombres, categorías y direcciones permanecen en tus dispositivos y se sincronizan con el Apple Watch con WatchConnectivity. No se envían a un servidor de PinO.
...
"""
        case "fr":
            return """
PinO enregistre les points que tu crées sur cet iPhone et Apple Watch. Lieux, noms, catégories et adresses restent sur tes appareils et se synchronisent avec l’Apple Watch via WatchConnectivity. Ils ne sont pas envoyés vers un serveur PinO.
...
"""
        default:
            return """
PinO saves pins you create on this iPhone and Apple Watch. Locations, names, categories, and addresses stay on your devices and sync to Apple Watch with WatchConnectivity. They are not uploaded to a PinO server.
...
PinO does not use advertising, analytics SDKs, or tracking. There is no account. Deleting the app removes that data from the device.
